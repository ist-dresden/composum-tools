class SaveAction extends ViewWidget {

  static selector = '.query-save .query-save_action';

  constructor(element) {
    super(element);
    this.$el.click(function (event) {
      event.preventDefault();
      const query = $('.browser-tool_query-form').serialize();
      const url = `${this.$el.data('uri')}?${query}`;
      window.open(url);
      return false;
    }.bind(this));
  ...
  onPathSelected(event, path) {
    const uri = this.$el.data('uri').replace(/\.([^.\/]+)(\/.*)?$/, `.$1${path}`);
    this.$el.data('uri', uri);
  }
}

CPM.widgets.register(SaveAction);

class QueryView extends ViewWidget {

  static selector = '.browser-tool_query';

  constructor(element) {
    super(element);
    this.profile = new Profile('query');
    this.$form = this.$('.browser-tool_query-form');
    this.$query = this.$form.find('input[name="query"]');
    this.$result = this.$('.browser-tool_query-result');
    this.$spinner = this.$('.browser-tool_query-spinner');
    this.$history = this.$('.query-history .dropdown-menu');
    this.history = this.profile.get('history') || [];
    this.historyMax = parseInt(this.$el.data('history-max') || '20');
    this.adjustHistory();
  ...
  onPathSelected(event, path) {
    ...
    this.$form.data('action', action.replace(/\.html(\/.*)?$/, `.html${path}`));
  }
  ...
  submitQuery() {
    ...
    this.loadContent(this.$result, this.formGetUrl(this.$form), ($element) => {
