class FavoritesToggle extends ViewWidget {
    ...
    onPathSelected(event, path) {
        ...
        if (this.history.length < 1 || this.history[this.history.length - 1] !== path) {
            ...
            this.applyToView(viewWidget => {
                if (viewWidget.$currentTab.attr('id') === FavoritesToggle.key.history) {
    ...
    adjustToggleAction() {
        if (this.currentPath) {
            const $toggle = $('.browser-tool_favorite-toggle');
...
class FavoritesView extends ViewWidget {
    ...
    static selector = '.browser-tool_favorites';
    ...
    constructor(element) {
        ...
        this.$content = this.$el.find('.browser-tool_favorites-content');
        this.showTab(this.profile.get('currentTab'), true);
        this.$el.find('.browser-tool_favorites-clear').click(this.clearFavorites.bind(this));
        this.$el.find('.browser-tool_favorites-groups .nav-link').click(this.selectTab.bind(this));
    }
    ...
    showTab(tabId, force) {
        this.$currentTab = this.$el.find('.browser-tool_favorites-groups .nav-link[id="' + tabId + '"]');
        if (this.$currentTab.length < 1 && force) {
            this.$currentTab = this.$el.find('.browser-tool_favorites-groups .nav-link').first();
        }
    ...
    renderFavorites() {
        let content = '';
        if (this.$currentTab.attr('id') === FavoritesToggle.key.history) {
